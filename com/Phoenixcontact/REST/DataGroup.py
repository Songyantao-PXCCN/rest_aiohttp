...
class DataGroup(object):
    ...
    def _RegisterGroup(self, variableNames, pathPrefix=RestConstant.PATHPREFIX):
        ...
        _status_code, _hearders, _text = self.loop.run_until_complete (self.Client._Http.asyncHttpAPI(httpMethod=RestConstant.POST,
    ...
    def _ReadGroupValues(ReadGroup):
        ...
        _status_code, _hearders, _text = ReadGroup.Client.loop.run_until_complete( ReadGroup.Client._Http.asyncHttpAPI(httpMethod=RestConstant.GET,
                                                                            function_uri=_url,
                                                                            payload=None))
        if _status_code == 200:
            _response = json.loads(_text)
    ...
    def _ReportGroups(self):
        if self.Client.accessToken == None:
            return
        _status_code, _hearders, _text = self.loop.run_until_complete (self.Client._Http.asyncHttpAPI(httpMethod=RestConstant.GET,
...
class AsyncRead(threading.Thread):
    def __init__(self, ReadGroup, threadID, name,poolSize):
        ...
        self.loop = self.ReadGroup.Client.loop
        self.pause = False
